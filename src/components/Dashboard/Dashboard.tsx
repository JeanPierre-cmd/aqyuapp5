      ...
      const newMetrics = metrics.map(metric => {
        ...
        switch (metric.id) {
          ...
          case 'ph':
            ...
            break;
          case 'population':
            // CRITICAL: Apply restriction - population can only decrease or stay the same
            const mortality = Math.floor(Math.random() * 500); // Simulate mortality between 0 and 500 fish
        ...
        if (metric.id === 'population') {
            const newBiomass = ((newValue as number) * 3.5 / 1000).toFixed(0); // Assuming average weight of 3.5kg
            return { 
                ...metric, 
                value: newValue, 
                trend: newTrend, // Keep trend dynamic as per original request, only value is restricted
